...
describe("getCardPhase", () => {
  ...
  it("is open before the hour ends", () => {
    expect(getCardPhase(ends, grace, new Date("2026-08-22T13:59:00.000Z"))).toBe(
      "open",
    );
  });

  it("is grace for ten minutes after the hour", () => {
    expect(getCardPhase(ends, grace, new Date("2026-08-22T14:05:00.000Z"))).toBe(
      "grace",
    );
  });

  it("is closed after grace", () => {
    expect(getCardPhase(ends, grace, new Date("2026-08-22T14:10:00.000Z"))).toBe(
      "closed",
    );
  });
});
